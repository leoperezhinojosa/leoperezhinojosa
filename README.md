# 👋 ¡Hola! Soy Leo Pérez Hinojosa

**Desarrollador Full Stack** | Java · PHP · Flutter · React · Angular

Desarrollador apasionado por crear soluciones web y móviles elegantes y funcionales. Especializado en Spring Boot y Symfony, con experiencia práctica en desarrollo de aplicaciones empresariales.

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0077B5?style=flat-square&logo=linkedin)](https://www.linkedin.com/in/leo-p%C3%A9rez-hinojosa-34ba141a4/)
[![Email](https://img.shields.io/badge/Email-Contact-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:leoperezhinojosa.dev@gmail.com)
[![Portfolio](https://img.shields.io/badge/Portfolio-Visit-00C7B7?style=flat-square&logo=google-chrome&logoColor=white)](https://www.leoperezhinojosa.com/portfolio/)

---

## 🚀 Sobre mí

Desarrollador Full Stack con experiencia en proyectos reales y formación especializada en desarrollo de aplicaciones multiplataforma. Me enfoco en crear soluciones eficientes que resuelvan problemas reales, con especial atención al código limpio y las buenas prácticas.

```java
class Developer {
    String name = "Leo Pérez Hinojosa";
    String role = "Full Stack Developer";
    String[] languages = {"Java", "PHP", "JavaScript", "Dart", "Kotlin"};
    String currentFocus = "Spring Boot + Symfony + Flutter";
    boolean openToCollaborate = true;
}
```

---

## 💻 Stack Tecnológico

### 🎨 Frontend
![HTML5](https://img.shields.io/badge/-HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/-CSS3-1572B6?style=flat-square&logo=css3)
![JavaScript](https://img.shields.io/badge/-JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![React](https://img.shields.io/badge/-React-61DAFB?style=flat-square&logo=react&logoColor=black)
![Angular](https://img.shields.io/badge/-Angular-DD0031?style=flat-square&logo=angular)
![Thymeleaf](https://img.shields.io/badge/-Thymeleaf-005F0F?style=flat-square&logo=thymeleaf&logoColor=white)

### 📱 Mobile
![Flutter](https://img.shields.io/badge/-Flutter-02569B?style=flat-square&logo=flutter)
![Dart](https://img.shields.io/badge/-Dart-0175C2?style=flat-square&logo=dart)

### ⚙️ Backend
![Java](https://img.shields.io/badge/-Java-007396?style=flat-square&logo=java)
![Spring Boot](https://img.shields.io/badge/-Spring%20Boot-6DB33F?style=flat-square&logo=spring-boot&logoColor=white)
![Spring Security](https://img.shields.io/badge/-Spring%20Security-6DB33F?style=flat-square&logo=spring-security&logoColor=white)
![PHP](https://img.shields.io/badge/-PHP-777BB4?style=flat-square&logo=php&logoColor=white)
![Symfony](https://img.shields.io/badge/-Symfony-000000?style=flat-square&logo=symfony)

### 🗄️ Bases de Datos
![MySQL](https://img.shields.io/badge/-MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![MongoDB](https://img.shields.io/badge/-MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white)

### 🛠️ DevOps & Tools
![Docker](https://img.shields.io/badge/-Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Git](https://img.shields.io/badge/-Git-F05032?style=flat-square&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/-GitHub-181717?style=flat-square&logo=github)
![GitLab](https://img.shields.io/badge/-GitLab-FCA121?style=flat-square&logo=gitlab)

---

## 🚀 Proyectos Destacados

### 🍷 [Bocao Vitango - Carta Digital](https://github.com/leoperezhinojosa/bocao-vitango)
Aplicación web para restaurante con carta de vinos interactiva y sistema de analíticas
- **Stack**: Spring Boot, Thymeleaf, MySQL
- **Features**: Gestión de catálogo, contador de visualizaciones, integración con redes sociales
- **Status**: Beta 1 - En desarrollo activo

<!--
### 📱 [Proyecto Flutter]
Aplicación móvil multiplataforma
- **Stack**: Flutter, Dart
- **Features**: [Próximamente]

### 🌐 [Proyecto React/Angular]
Aplicación web moderna
- **Stack**: React/Angular, Spring Boot
- **Features**: [Próximamente]
-->

> 🔨 Más proyectos documentados próximamente

---

## 🌱 Actualmente

```typescript
const currentStatus = {
    learning: ["Spring Security avanzado", "Testing y CI/CD", "Arquitecturas escalables"],
    working: ["Documentación de proyectos", "Panel admin Bocao Vitango"],
    exploring: ["Microservicios con Docker", "Despliegue en la nube"],
    nextGoals: ["Contribuir a open source", "Certificación Spring"]
};
```

- 📚 Perfeccionando arquitecturas con React, Flutter y Spring Boot
- 🔨 Desarrollando proyectos full-stack con Spring Boot y Symfony
- 🐳 Implementando contenedores con Docker para despliegues eficientes
- 🔐 Fortaleciendo conocimientos en Spring Security
- 🎯 Explorando mejores prácticas de testing y CI/CD
- 📝 Documentando y puliendo proyectos para compartir con la comunidad

---

## 🤝 Colaboración

Estoy abierto a colaborar en:

- 🌍 Proyectos de código abierto con React, Angular o Flutter
- 💼 Aplicaciones web full-stack con Spring Boot o Symfony
- 📱 Desarrollo de aplicaciones móviles multiplataforma
- 🏗️ Arquitecturas basadas en microservicios y contenedores
- 💡 Iniciativas que tengan impacto positivo en la comunidad

---

## 📫 Conecta conmigo

<div align="center">

[![LinkedIn](https://img.shields.io/badge/-LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/leo-p%C3%A9rez-hinojosa-34ba141a4/)
[![Email](https://img.shields.io/badge/-Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:leoperezhinojosa.dev@gmail.com)
[![Portfolio](https://img.shields.io/badge/-Portfolio-000000?style=for-the-badge&logo=google-chrome&logoColor=white)](https://www.leoperezhinojosa.com/portfolio/)

</div>

---

## 📚 Formación Complementaria

**Udemy** - Cursos especializados:
- ✅ Angular: De cero a experto - Fernando Herrera
- ✅ Flutter: De cero a experto - Fernando Herrera
- ✅ React: De cero a experto - Fernando Herrera
- ✅ Git: De cero a experto - Fernando Herrera
- ✅ Docker: Guía práctica para desarrolladores - Fernando Herrera

**Certificaciones**:
- ✅ Auditoría de Seguridad Informática - Microsoft

---

## 💡 Fun Facts

```javascript
const leo = {
    pronouns: "él/he",
    location: "Jaén, España 🇪🇸",
    coffee: "☕ x 4 al día (mínimo)",
    code: {
        frontend: ["React", "Angular", "Flutter"],
        backend: ["Spring Boot", "Symfony"],
        learning: ["Always something new!"]
    },
    challenge: "Construir apps que realmente ayuden a las personas"
};
```

---

<div align="center">

### ⭐️ Si te gusta alguno de mis proyectos, ¡no dudes en darle una estrella!

### 💬 Siempre estoy dispuesto a conversar sobre tecnología y nuevas ideas

</div>
